...
namespace EthereumTransactionSearch.Controllers
{
    ...
    {
        [HttpGet("{blockNumber}")]
        public async Task<IEnumerable<EthereumTransaction>> GetTransactions(string blockNumber, [FromQuery]string address)
        {
            ...
            {
                return await Task.FromResult(ethDetail.Where(t => t.From == address));
            }

            return await Task.FromResult(ethDetail);
        }
    }
}